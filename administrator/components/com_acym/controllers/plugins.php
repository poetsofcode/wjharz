...
class PluginsController extends acymController
{
    ...
    {
        $pluginClass = acym_getVar('string', 'plugin');
    ...
    {
        $pluginFolderName = $this->getPluginClassAjaxCustomView();

        $customLayoutPath = ACYM_CUSTOM_PLUGIN_LAYOUT.$pluginFolderName.'.html';

        $customView = '<table></table>';

        if (file_exists($customLayoutPath)) $customView = acym_fileGetContent($customLayoutPath);
        if (empty($customView)) $customView = '<table></table>';
