    ...
    public function generateSettings(&$plugin)
    {
        ...
        foreach ($plugin->settings as $key => $field) {
            ...
            if ($field['type'] == 'checkbox') {
                ...
                $text .= acym_switch($name, $field['value'], $field['label'], [], 'shrink');
            } elseif ($field['type'] == 'select') {
                $text .= '<label class="cell shrink">'.$field['label'].'</label>';
                $text .= acym_select($field['data'], $name, $field['value'], 'class="acym__select"');
            } elseif ($field['type'] == 'multiple_select') {
                $text .= '<label class="cell shrink">'.$field['label'].'</label>';
                $text .= acym_selectMultiple($field['data'], $name, empty($field['value']) ? [] : $field['value'], ['class' => 'acym__select']);
            } elseif ($field['type'] == 'text') {
                $text .= '<label class="cell shrink">'.$field['label'].'</label>';
                $text .= '<input type="text" name="'.$name.'" value="'.acym_escape($field['value']).'" class="cell shrink">';
            } elseif ($field['type'] == 'radio') {
                $text .= '<p class="cell">'.$field['label'].'</p>';
                $text .= acym_radio($field['data'], $name, $field['value']);
            } elseif ($field['type'] == 'date') {
                $text .= '<label class="cell shrink">'.$field['label'].'</label>';
                $text .= acym_dateField($name, $field['value'], 'cell shrink');
                ...
                $modalContent = '<div id="'.$idCustomView.'" class="cell grid-x acym__plugins__installed__custom-view">
                                    ...
                                    <p class="cell">'.acym_translation('ACYM_CUSTOM_VIEW_NEED_TAGS').'</p>
                                    <vue-prism-editor class="cell acym__plugins__installed__custom-view__code" v-model="code" :language="language" lineNumbers="true"></vue-prism-editor>
                ...
                $text .= acym_modal(acym_translation('ACYM_EDIT_CUSTOM_VIEW'), $modalContent, null, 'acym-data-plugins-id="'.$idCustomView.'" acym-data-plugin-folder="'.$this->name.'"', 'class="cell button"');
