			...
			foreach ($items as $key => $item) {
				...
				if(!$hide_thumbnail) {
					...
					if($resource != 'k2' && $item->post_format=='gallery') {
						if(count((array) $item->imagegallery->images)) {
							...
							$output .= '<a href="'. $item->link .'" ><img class="sppb-img-responsive" src="'. $item->image_thumbnail .'" alt="'. $item->title .'" ></a>';
						}
						...
						if(isset($image) && $image) {
							$output .= '<a class="sppb-article-img-wrap" href="'. $item->link .'" ><img class="sppb-img-responsive" src="'. $image .'" alt="'. $item->title .'" ></a>';
						}
					}
				}
				...
				$output .= '<div class="sppb-article-info-wrap">';
					...
					if($show_author || $show_category || $show_date) {
						...
						if($show_date) {
							$output .= '<span class="sppb-meta-date" >' . Jhtml::_('date', $item->publish_up, 'DATE_FORMAT_LC3') . '</span>';
						}
						...
						if($show_category) {
							...
							$output .= '<span class="sppb-meta-category"><a href="'. $item->catUrl .'" >' . $item->category . '</a></span>';
						}

						if($show_author) {
							$author = ( $item->created_by_alias ?  $item->created_by_alias :  $item->username);
							$output .= '<span class="sppb-meta-author" >' . $author . '</span>';
						}
					...
					if($show_readmore) {
						$output .= '<a class="sppb-readmore" href="'. $item->link .'" >'. $readmore_text .'</a>';
					}
			...
			if($link_articles) {
				...
				if (count($icon_arr) === 1) {
					...
					if ($font_awesome_version == '4') {
						$$all_articles_btn_icon = 'fa ' . $icon_arr[1];
					}
				}
