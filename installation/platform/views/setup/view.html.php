...
class AngieViewSetup extends AView
{
	...
	/** @var array  */
	public $removeHandlerOptions = [];

	/** @var array  */
	public $updateHandlerOptions = [];
	...
	public function onBeforeMain()
	{
		...
		// No handler in original .htaccess file? No need to display the option
		if (!$model->hasAddHandler())
		{
			$this->removeHandlerOptions['disabled'] = 'disabled="disabled"';
			$this->removeHandlerOptions['checked']  = '';
			$this->removeHandlerOptions['help']     = 'SETUP_LBL_SERVERCONFIG_NONEED_HELP';

			$this->updateHandlerOptions['disabled'] = 'disabled="disabled"';
			$this->updateHandlerOptions['checked']  = '';
			$this->updateHandlerOptions['help']     = AText::_('SETUP_LBL_SERVERCONFIG_NONEED_HELP');
		}
