...
class AngieModelJoomlaSetup extends AngieModelBaseSetup
{
	...
	private function applyServerconfigchanges()
	{
		...
		elseif($this->input->get('updateaddhandler'))
		{
			...
			// If we're here, we should always have something to do, but let's be safe than sorry
			if ($newRules)
			{
				$this->updateHandlerRules($newRules, APATH_ROOT.'/htaccess.bak');
			}
		}
	}
	...
	private function removeAddHandler()
	{
		...
		foreach ($files as $file)
		{
			...
			if (!$this->updateHandlerRules('', $target_file))
			{
				return false;
			}
		}
	 ...
	 */
	private function updateHandlerRules($newValues, $targetFile)
	{
	 ...
	 * @return string
	 */
	private function getHandlerRules($targetFile)
	{
		if (!file_exists($targetFile))
		{
			return '';
	...
	public function hasAddHandler()
	{
		...
		foreach ($files as $file)
		{
			...
			if (strpos($contents, 'AddHandler') !== false || (strpos($contents, 'SetHandler') !== false))
			{
				return true;
			}
		}
